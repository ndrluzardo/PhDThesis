...
% For the future: try getting rid of the separate associative unit for the
% gap. It would be more parsimonious.

close all

% Simulation is divided into three timed intervals:
% 1 - The ITI; 1 timer and 1 V value.
% 2 - the CS; 1 timer and 2 V values.
% 3 - the 10 second gap between CS offset and US. 1 timer and 2 V values.
...
%     A_CX=NoITIUS.Tra.CX.A1(trial); % A trial value for CX
    V_CX=ITIUS.Tra.CX.V(trial); % V trial value for CX
%     %--
%     
...
%     %---
    ...
    %--Obtain A and V values from memory
    A_CX=ITIUS.Tra.CX.A_CS(trial); % A trial value for CX
    A_CS=ITIUS.Tra.CS.A(trial); % A trial value for CS
    V_CS=ITIUS.Tra.CS.V(trial); % V trial value for CS
    %--
    ...
    %---Move every A and V value 1 trial forward
    ITIUS.Tra.CX.A_CS(trial+1)=A_CX; % A trial value for CX
    ITIUS.Tra.CS.A(trial+1)=A_CS; % A trial value for CS
    ITIUS.Tra.CS.V(trial+1)=V_CS; % V trial value for CS
    %---
    %---End of CS
    ...
    %--Obtain A and V values from memory
    A_CX=ITIUS.Tra.CX.A_G(trial); % A trial value for CX
    A_CG=ITIUS.Tra.CG.A(trial); % A trial value for CG
    V_CG=ITIUS.Tra.CG.V(trial); % V trial value for CG
    %--
    ...
    for t2=1:interval_length
        ...
        %---CR
        CR(t+t2)=max(0, x_CX(t2)*V_CX);%[x_CX(t2) x_CG(t2)]*[V_CX V_CG]');
        %---
    ...
    %---Move every A and V value 1 trial forward
    ITIUS.Tra.CX.A_G(trial+1)=A_CX; % A trial value for CX
    ITIUS.Tra.CX.V(trial+1)=V_CX; % V trial value for CX
    ITIUS.Tra.CG.A(trial+1)=A_CG;
    ITIUS.Tra.CG.V(trial+1)=V_CG;
    ITIUS.Tra.CR(trial,:)=CR;
    %---
...
MeanCR=mean(ITIUS.Tra.CR((end-40):end,:),1);
%---
...
subplot(2,3,1)
plot(1:length(ITIUS.Tra.CS.V),ITIUS.Tra.CS.V,1:length(ITIUS.Tra.CX.V),ITIUS.Tra.CX.V)
legend('V CS','V CX')

subplot(2,3,2)
plot(1:length(ITIUS.Tra.CX.A_ITI)-10,1./ITIUS.Tra.CX.A_ITI(11:end))
legend('CX time est. to CS: 340')

subplot(2,3,3)
plot(1:length(ITIUS.Tra.CX.A_CS)-10,1./ITIUS.Tra.CX.A_CS(11:end))
legend('CX time est. to CS off: 120')

subplot(2,3,4)
plot(1:length(ITIUS.Tra.CS.A)-10,1./ITIUS.Tra.CS.A(11:end))
legend('CS time est. to CS off: 120')

subplot(2,3,5)
plot(1:length(ITIUS.Tra.CX.A_G)-10,1./ITIUS.Tra.CX.A_G(11:end))
