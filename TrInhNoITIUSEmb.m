...
N_CS3=normrnd(0,1,ceil((CX_dur/h)*cycle_num),1); % noise for CS2
%---
...
for trial=1:cycle_num
    ...
    V_CX=NoITIUS.Emb.CX.V(trial); % V trial value for CX
    %--
    ...
    %--Obtain A and V values from memory
    A_CX=NoITIUS.Emb.CX.A_CS1(trial); % A trial value for CX
    A_CS=NoITIUS.Emb.CS.A1(trial); % A trial value for CS
    V_CS=NoITIUS.Emb.CS.V(trial); % V trial value for CS
    %--
    ...
    %---Move every A and V value 1 trial forward
    NoITIUS.Emb.CX.A_CS1(trial+1)=A_CX; % A trial value for CX
    NoITIUS.Emb.CS.A1(trial+1)=A_CS; % A trial value for CS
    %---
    %---End of CS, 1st part
...
%     P_CX=zeros(1,interval_length);
%     P_CS=zeros(1,interval_length);
%     P_CS2=zeros(1,interval_length);
%     
%     x_CX=zeros(1,interval_length);
%     x_CS=zeros(1,interval_length);
%     x_CS2=zeros(1,interval_length);
%     %--
%     
%     %--Obtain A and V values from memory
%     A_CX=NoITIUS.Emb.CX.A_CS2(trial); % A trial value for CX
%     A_CS=NoITIUS.Emb.CS.A2(trial); % A trial value for CS
%     A_CS2=NoITIUS.Emb.CS.A3(trial); % A trial value for CS
%     V_CS2=NoITIUS.Emb.CS.V2(trial);
%     %--
%     
...
%         P_CS2(t2+1)=min(DDM( P_CS2(t2), A_CS2, h, m, N_CS3(counterDDMCS2) ), 3);
%         
%         % max ensures the minimum value the accumulator can reach is
%         % Aini. This avoids division by zero later.
%         P_CX(t2+1)=max(P_CX(t2+1), Aini);
%         P_CS(t2+1)=max(P_CS(t2+1), Aini);
%         P_CS2(t2+1)=max(P_CS2(t2+1), Aini);
%         
%         %---Stimuli representations (RBFs)
%         x_CX(t2)=CStrace(P_CX(t2+1),mu,sigma,tau_x,1,x_CX(t2),h);
%         x_CS(t2)=CStrace(P_CS(t2+1),mu,sigma,tau_x,1,x_CS(t2),h);
%         x_CS2(t2)=CStrace(P_CS2(t2+1),mu,sigma,tau_x,1,x_CS2(t2),h);
%         %---
%         
%         %---CR
%         CR(t+t2)=max(0,[x_CS(t2) x_CX(t2) x_CS2(t2)]*[V_CS V_CX V_CS2]');
%         %---
%         
%     end
%     
...
%     A_CS2=A_CS2+A_CS2*alpha_t*(1-P_CS2(t2+1))/P_CS2(t2+1);
%     %---
%     
%     %---V update
%     V_CX=RW([V_CX V_CS V_CS2],alpha_E,[x_CX(t2) x_CS(t2) x_CS2(t2)],0,A_CX,P_CX(t2+1));
%     V_CS=RW([V_CS V_CS2 V_CX],alpha_E,[x_CS(t2) x_CS2(t2) x_CX(t2)],0,A_CS,P_CS(t2+1));
%     V_CS2=RW([V_CS2 V_CX V_CS],alpha_E,[x_CS(t2) x_CX(t2) x_CX(t2)],0,A_CS2,P_CS2(t2+1));
%     %---
    
    %---Move every A and V value 1 trial forward
    NoITIUS.Emb.CX.A_CS2(trial+1)=A_CX; % A trial value for CX
    NoITIUS.Emb.CS.A2(trial+1)=A_CS; % A trial value for CS
    NoITIUS.Emb.CX.V(trial+1)=V_CX; % V trial value for CX
    NoITIUS.Emb.CS.V(trial+1)=V_CS; % V trial value for CS
%     NoITIUS.Emb.CS.A3(trial)=A_CS2; % A trial value for CS
%     NoITIUS.Emb.CS.V2(trial)=V_CS2;
    NoITIUS.Emb.CR(trial,:)=CR;
    %---
...
MeanCR=mean(NoITIUS.Emb.CR((end-40):end,:),1);
%---
...
subplot(2,4,1)
plot(1:length(NoITIUS.Emb.CS.V),NoITIUS.Emb.CS.V,1:length(NoITIUS.Emb.CX.V),NoITIUS.Emb.CX.V)
legend('V CS','V CX')

subplot(2,4,2)
plot(1:length(NoITIUS.Emb.CX.A_ITI)-10,1./NoITIUS.Emb.CX.A_ITI(11:end))
legend('CX time est. to CS: 350')

subplot(2,4,3)
plot(1:length(NoITIUS.Emb.CX.A_CS1)-10,1./NoITIUS.Emb.CX.A_CS1(11:end))
legend('CX time est. to reward: 110')

subplot(2,4,4)
plot(1:length(NoITIUS.Emb.CS.A1)-10,1./NoITIUS.Emb.CS.A1(11:end))
legend('CS time est. to reward: 110')

subplot(2,4,5)
plot(1:length(NoITIUS.Emb.CX.A_CS2)-10,1./NoITIUS.Emb.CX.A_CS2(11:end))
legend('CX time est. to CS off: 10')

subplot(2,4,6)
plot(1:length(NoITIUS.Emb.CS.A2)-10,1./NoITIUS.Emb.CS.A2(11:end))
